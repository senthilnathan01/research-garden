---
title: OpenAI Challenge Parameter Golf
description: Bets, logs, references, and candidate submissions for the parameter golf challenge.
visibility: public
publication_status: published
draft: false
content_type: project
validated: 2026-07-25
artifacts:
  - label: Challenge repository
    href: https://github.com/openai/parameter-golf
    kind: code
  - label: OpenAI retrospective
    href: https://openai.com/index/what-parameter-golf-taught-us/
    kind: results
tags:
  - open-ai-challenge-parameter-golf
  - project
  - challenge
---

This is the working folder for the parameter golf challenge.

Live bets, dated notes, references, and submission candidates stay here.

## Current status

- The folder structure was simplified on 2026-03-25.
- There are now four working areas instead of many note types.
- No active bet files or candidate submissions are recorded yet.

## Working areas

- [Bets](./bets/)
- [Log](./log/)
- [References](./references/)
- [Submissions](./submissions/)
- [Templates](./templates/)

## Next actions

1. Write the first live bet in [Bets](./bets/) with the [bet template](./templates/bet-template/).
2. Add a dated [log](./log/) entry for the latest constraint, experiment, or result.
3. Put the challenge rules, links, and repository notes in [References](./references/).
