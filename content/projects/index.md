---
title: Projects
description: Entry point for all research projects stored in this garden.
tags:
  - projects
  - index
---

Each project in this repository should live under `content/projects/<project-slug>/`.

The goal is to keep every project self-contained while preserving cross-links between projects when ideas, papers, or experiments intersect.

## Current placeholder project

- [[projects/open-ai-challenge-parameter-golf/index|open-ai-challenge-parameter-golf]]

## Add a new project

1. Duplicate the placeholder project folder.
2. Rename the folder to the new slug.
3. Update each page title, description, and internal link.
4. Start adding real content under `papers/`, `notes/`, `resources/`, and `experiments/`.

If you want starter copy for new pages, use [[templates/index|the template library]].
